...
function copyCSS(cb) {
	jdev.files.css.forEach((item) => {
		return gulp
			.src(jdev.src.css + item.src)
			.pipe(global.checkChanged === true ? $.changed(jdev.dist.css + item.dest) : gutil.noop())
			.pipe(
				browserSync.reload({
					stream: true,
				})
			)
			.pipe(gulp.dest(jdev.dist.css + '/plugins/' + item.dest))
	})
	cb()
}
