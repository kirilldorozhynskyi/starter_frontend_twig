...
function copyMedia(cb) {
	jdev.files.media.forEach((item) => {
		return gulp
			.src(jdev.src.media + item.src)
			.pipe(global.checkChanged === true ? $.changed(jdev.dist.media + item.dest) : gutil.noop())
			.pipe(
				browserSync.reload({
					stream: true,
				})
			)
			.pipe(gulp.dest(jdev.dist.media + item.dest))
	})
	cb()
}
